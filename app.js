...
const STORAGE={trades:"btc32_v16_trades",cash:"btc32_v16_cash",reserved:"btc32_v16_reserved",pos:"btc32_v16_pos",initialized:"btc32_v16_initialized",snapshot:"btc32_v16_snapshot"};
const DB_NAME="btc32_persistence_v16";const DB_STORE="snapshots";const DB_KEY="account";const CACHE_NAME="btc-v3-persistent-account-v16";const CACHE_KEY="./.btc-v3-account-backup-v16.json";const OLD_CACHE_NAMES=["btc-v3-persistent-account-v14"];
const OLD_DB_NAMES=["btc32_persistence_v14"];
...
function updateUI(){
 ...
 $("cash").textContent="$"+fmt(state.cash,2);$("pool").textContent="$"+fmt(state.pool,2);$("balance").textContent="$"+fmt(equity(),2);renderAccountStats();
...
function snapshot(){return {version:16,savedAt:Date.now(),trades:state.trades.slice(),cash:state.cash,reservedMargin:state.reservedMargin,pos:state.pos}}
...
async function load(){
 ...
 if(candidates.length){candidates.sort((a,b)=>(Number(b.savedAt)||0)-(Number(a.savedAt)||0));applySnapshot(candidates[0]);save()}
   ...
   if(!applySnapshot(legacy)){state.trades=[];state.cash=1000;state.startingCash=1000;state.reservedMargin=0;state.pos=null;state.pool=1000;save()}
 }
 state.auto=true;renderTrades();renderAuto();renderAccountStats();storageStatus();
}
...
function openPos(side){
 ...
 let margin=Math.min(state.cash/feeBuffer,state.startingCash/feeBuffer); if(margin<10)return;
 ...
 state.pos={tradeId:id,side,entry,margin,marginBalance:margin,leverage,mode:'全仓',contracts,opened:now,
   openScore:state.lastScore,entryFee:fee,entryFunding:0,fundingPaid:0,peakMove:0,maxFavorable:0,maxAdverse:0};
