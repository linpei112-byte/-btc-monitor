...
const DB_NAME="btc32_v11_persistence";const DB_STORE="snapshots";const DB_KEY="account";const CACHE_NAME="btc-v3-11-account-backup";const CACHE_KEY="./.btc-v3-account-backup.json";
...
async function cacheGet(){try{if(!window.caches)return null;const c=await caches.open(CACHE_NAME);const r=await c.match(CACHE_KEY);if(!r)return null;const x=await r.json();return x||null}catch(e){return null}}
...
function snapshot(){return {version:11,savedAt:Date.now(),trades:state.trades.slice(),cash:state.cash,reservedMargin:state.reservedMargin,pos:state.pos}}
function save(){const snap=snapshot();state.lastSavedAt=snap.savedAt;try{localStorage.setItem(STORAGE.trades,JSON.stringify(snap.trades));localStorage.setItem(STORAGE.cash,String(snap.cash));localStorage.setItem(STORAGE.reserved,String(snap.reservedMargin));localStorage.setItem(STORAGE.pos,JSON.stringify(snap.pos));localStorage.setItem(STORAGE.snapshot,JSON.stringify(snap));localStorage.setItem(STORAGE.initialized,"1")}catch(e){};void idbPut(snap);void cachePut(snap)}
function renderAuto(){const b=$("auto");if(!b)return;b.textContent=state.auto?"关闭":"开启";b.className=state.auto?"on":"on off"}
function applySnapshot(snap){if(!snap||!Array.isArray(snap.trades)||!Number.isFinite(Number(snap.cash))||!Number.isFinite(Number(snap.reservedMargin)))return false;state.trades=snap.trades;state.cash=Number(snap.cash);state.reservedMargin=Number(snap.reservedMargin);state.pos=snap.pos||null;state.pool=Math.min(3000,Math.max(0,state.cash*0.3));return true}
async function load(){
 let candidates=[];
 void requestPersistentStorage();
 try{const raw=localStorage.getItem(STORAGE.snapshot);if(raw){const x=JSON.parse(raw);if(x&&Array.isArray(x.trades)&&Number.isFinite(Number(x.cash))&&Number.isFinite(Number(x.reservedMargin)))candidates.push(x)}}catch(e){}
 ...
 state.auto=false;renderTrades();renderAuto();
}
setInterval(()=>{if(state.ready)save()},5000);
