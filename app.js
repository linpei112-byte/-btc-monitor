...
const STORAGE={trades:"btc32_v5_trades",cash:"btc32_v5_cash",reserved:"btc32_v5_reserved",pos:"btc32_v5_pos",initialized:"btc32_v5_initialized"};
...
function save(){try{localStorage.setItem(STORAGE.trades,JSON.stringify(state.trades.slice(0,500)));localStorage.setItem(STORAGE.cash,String(state.cash));localStorage.setItem(STORAGE.reserved,String(state.reservedMargin));localStorage.setItem(STORAGE.pos,JSON.stringify(state.pos));localStorage.setItem(STORAGE.initialized,"1");}catch(e){}}
function renderAuto(){const b=$("auto");if(!b)return;b.textContent=state.auto?"关闭":"开启";b.className=state.auto?"on":"on off"}
function load(){try{const initialized=localStorage.getItem(STORAGE.initialized)==="1";if(!initialized){state.trades=[];state.cash=10000;state.reservedMargin=0;state.pos=null;state.pool=3000;localStorage.setItem(STORAGE.initialized,"1");save()}else{state.trades=JSON.parse(localStorage.getItem(STORAGE.trades)||"[]");state.cash=Number(localStorage.getItem(STORAGE.cash)||10000);state.reservedMargin=Number(localStorage.getItem(STORAGE.reserved)||0);state.pos=JSON.parse(localStorage.getItem(STORAGE.pos)||"null");if(!Number.isFinite(state.cash))state.cash=10000;if(!Number.isFinite(state.reservedMargin))state.reservedMargin=0;state.pool=Math.min(3000,Math.max(0,state.cash*0.3));}}catch(e){state.trades=[];state.cash=10000;state.reservedMargin=0;state.pos=null;state.pool=3000}renderTrades();renderAuto()}
